...
func TestCapabilityConfigValidatorContentTracing(t *testing.T) {
	newConfig := func(contentTracing attr.Value, retentionType string) types.Object {
		...
		for name, typ := range capabilityConfigAttributeTypes() {
			attrs[name] = newNullOfType(typ)
		}
		attrs["content_tracing"] = contentTracing
		attrs["data_retention"] = retention
		return types.ObjectValueMust(capabilityConfigAttributeTypes(), attrs)
	}
...
// newNullOfType returns a null value of the given attribute type.
func newNullOfType(typ attr.Type) attr.Value {
	switch t := typ.(type) {
	...
	default:
		panic(fmt.Sprintf("newNullOfType: unhandled type %T", typ))
	}
}
